...
```{r}
wildfire_shp_data <- st_read(here("/Users/takaz/OneDrive/DS-421NEW/data/LANDSAT_USA_contiguous_and_Hawaii_7d.shp"))
```
...
ggplot(wildfire_counts, aes(x = ACQ_DATE, y = n_fires)) +
  geom_line(color = "firebrick") + #ADD RED COLORING TO LINES
  geom_point(color = "firebrick") + #ADD RED COLORING TO POINTS
  labs(
    x = "Date",
    y = "Number of detected fires",
    title = "Daily number of LANDSAT-detected fires (7-day FIRMS data)"
  ) +
  theme_minimal()
```
...
```{r}
wildfire_counts_conf <- wildfire_shp_data |>
  group_by(ACQ_DATE, CONFIDENCE) |>
  summarise(n_fires = n(), .groups = "drop")

ggplot(wildfire_counts_conf,
       aes(x = ACQ_DATE, y = n_fires, fill = CONFIDENCE)) +
  geom_col() +
  labs(
    x = "Date",
    y = "Number of detected fires",
    fill = "Confidence",
    title = "Daily LANDSAT fire detections by confidence"
  ) +
  theme_minimal()
```
...
mapview(
  wildfire_shp_data,
  zcol = "CONFIDENCE", #COLOR BY THE CONFIDENCE
  layer.name = "Fire detections",
  legend = TRUE
)
```
...
```{r}
library(viridisLite) #Library for confidence colors on heatmap

wildfire_high <- wildfire_shp_data |>
  filter(CONFIDENCE == "high")

mapboxgl(
  style  = mapbox_style("dark"),
  center = c(mean(wildfire_high$LONGITUDE),
             mean(wildfire_high$LATITUDE)),
  zoom   = 2
) |>
  add_heatmap_layer(
    id    = "fire_heatmap_high",
    source = wildfire_high,
    heatmap_radius = 2,
    heatmap_intensity = mapgl::interpolate(
      property = "zoom",
      values   = c(0, 9),
    ...
    heatmap_color = mapgl::interpolate(
      ...
      values   = seq(0, 1, 0.2),
      stops    = c("transparent",
                   viridisLite::viridis(5, option = "inferno"))
    ),
    heatmap_opacity = 0.8
  )
```
...
```{r}
unzip("/Users/takaz/OneDrive/DS-421NEW/data/FirespotArea_LANDSAT_USA_contiguous_and_Hawaii_7d.kmz", exdir = "/Users/takaz/OneDrive/DS-421NEW/data/fire_kml")
```

```{r}

wildfire_area_data <- st_read(
  "/Users/takaz/OneDrive/DS-421NEW/data/fire_kml/LANDSAT_USA_contiguous_and_Hawaii_7d_1753145203282.kml",
...
library(sf)
library(ggplot2)
library(viridis)       # optional, for a nice color scale
library(lubridate)     # optional, for date handling

# 1. Read CSV into a data frame
wildfire_csv <- read_csv("/Users/takaz/OneDrive/DS-421NEW/data/LANDSAT_USA_contiguous_and_Hawaii_7d.csv")

# Inspect column names if you're unsure (run once in your console):
# names(wildfire_csv)

# 2. Convert to sf object with coordinates and CRS
# FIRMS uses WGS84 (EPSG:4326) with LONGITUDE = x, LATITUDE = y

# 2. Convert to sf object with coordinates and CRS
wildfire_sf <- wildfire_csv |>
  mutate(
    # Use the existing acq_date (already Date); optionally create ACQ_DATE alias
    ACQ_DATE   = acq_date,
    day_of_week = wday(acq_date, label = TRUE),
    is_weekend  = day_of_week %in% c("Sat", "Sun")
  ) |>
  st_as_sf(
    coords = c("longitude", "latitude"),  # note lowercase names from CSV
    crs    = 4326
  )

# 3. Creative map: facet by confidence × weekend/weekday
ggplot(wildfire_sf) +
  geom_point(
    aes(
      x = st_coordinates(geometry)[, 1],
      y = st_coordinates(geometry)[, 2],
      color = confidence          # lowercase column name from CSV
    ),
    alpha = 0.6,
    size  = 0.7
  ) +
  facet_grid(is_weekend ~ confidence, labeller = label_both) +
  scale_color_manual(
    values = c(low = "forestgreen", medium = "orange", high = "red"),
    name   = "Confidence"
  ) +
  ...
  labs(
    title    = "LANDSAT Fire Detections (7-day, USA + Hawaii)",
    subtitle = "Faceted by confidence and weekend vs weekday",
    x        = "Longitude",
    y        = "Latitude",
    caption  = "Source: NASA FIRMS (LANDSAT 30 m)"
  ) +
  theme(
